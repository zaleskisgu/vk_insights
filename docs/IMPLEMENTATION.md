...
| `GET /report?group=&from=&to=` | JSON дашборда: `meta` (в т.ч. **`generated_at`**, **`truncated`**, **`posts_limit`** при неполной выборке стены), `summary`, `daily`, `top_posts`, `content_types`. Источник данных: **мок** или **живая стена VK** (см. ниже). Параметры: **`group`**, **`from`**, **`to`** (рекомендуемый формат даты — **`YYYY-MM-DD`**). Под **`middleware('throttle:30,1')`**. | В ТЗ — `POST /analyze` и путь с `groupId`; здесь **`GET`** с query. |
| `POST /report/export` | Тело `group`, `from`, `to`, **`format`**: `json` \| `csv`. JSON/CSV = дашборд + **`all_posts`**. Ответ стримится (**`StreamedResponse`**), без сборки всего тела в память для CSV. Под **`throttle:30,1`**. | Формат запроса свой. |
| `POST /report/posts` | Постраничный список постов: те же `group`/`from`/`to`, опционально `page`, `per_page`, `sort`, `order`, `q`, `type`; ответ `{ data, meta }`. Данные из того же снимка, что и отчёт (мок или кэшированная выборка стены). Под **`throttle:30,1`**. | В ТЗ отдельного эндпоинта нет. |
...
## Параметры периода и валидация

- Запросы отчёта проходят через Form Request: базовый класс [`ReportPeriodRequest`](../app/Http/Requests/ReportPeriodRequest.php), реализации [`ReportRequest`](../app/Http/Requests/ReportRequest.php), [`ReportPostsRequest`](../app/Http/Requests/ReportPostsRequest.php), [`ReportExportRequest`](../app/Http/Requests/ReportExportRequest.php).
- **`from`** и **`to`** — обязательные поля, парсятся правилом **`date`**, **`to`** не раньше **`from`**. Верхняя граница «не позже сегодня» и проверка максимальной длины периода (**`vk.period_max_days`**, env **`VK_PERIOD_MAX_DAYS`**) выполняются в **`withValidator()->after`** с опорой на **`config('vk.timezone')`** (по умолчанию **`Europe/Moscow`** из **`VK_TIMEZONE`**). Так «сегодня» для пользователя согласовано с отображением постов и не ломается рассинхрон **`app.timezone`** (часто UTC) и локали браузера.
- На главной форме [`StartScreen.vue`](../resources/js/screens/StartScreen.vue) даты отправляются в query как строки **`YYYY-MM-DD`** (локальный календарный день), без **`Date.prototype.toISOString()`**, чтобы выбранный в календаре день совпадал с переданной строкой.
...
- [`ReportController`](../app/Http/Controllers/ReportController.php): **`ReportRequest`**, сервис.
- [`ReportService`](../app/Services/ReportService.php) (`getReportData` / `getExportData`): через **`DashboardFixtureFactory::create()`** — мок или live; **`meta`**: `name` / `screen_name` из VK с фолбэком на **`VkGroupInputParser`**; **`group_query`**, **`owner_id`**, **`members_count`**, **`photo_200`**, **`generated_at`** (часовой пояс **`vk.timezone`**), **`truncated`**, **`posts_limit`** при необходимости.
- [`ReportPostsController`](../app/Http/Controllers/ReportPostsController.php) + [`ReportPostsService::listPage`](../app/Services/Posts/ReportPostsService.php): **`ReportPostsRequest`**, тот же **`DashboardFixtureFactory`**, что и отчёт; фильтры, сортировка, пагинация в PHP.
- [`ReportExportController`](../app/Http/Controllers/ReportExportController.php): **`ReportExportRequest`**, **`ReportService::getExportData()`** — дашборд + **`all_posts`**; ответ стримом (**`ReportCsvExporter::streamTo`** для CSV).
...
- **VK:** **`VK_SERVICE_TOKEN`**, **`VK_API_VERSION`**, **`VK_USE_MOCK`**, **`VK_INTEGRATION_TEST_GROUP_ID`**, **`VK_CACHE_TTL`**, **`VK_TIMEZONE`**, **`VK_PERIOD_MAX_DAYS`**, **`VK_WALL_MAX_PAGES`**, **`VK_WALL_PAGE_SIZE`** — [`config/vk.php`](../config/vk.php), [`.env.example`](../.env.example).
